...
const Appointments = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      // seeting date by index
      let currentDate = new Date(today);
      currentDate.setDate(today.getDate() + i);

      // setttinf endtime of the date with index
          ...
          <div className="flex gap-3 items-center w-full overflow-x-scroll mt-4">
            {docSlot.length &&
              docSlot.map((item, index) => {
                 return <div key={index} onClick={() => setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer transition-all duration-150 ${slotIndex === index ? "bg-primary text-white" : "border border-gray-300"}`}>
